...
def updateAccess(group):
    ...
    c1 = db.cursor()
    c1.execute('select entity_id,conditional,auto_2fa from access_control where end_time is null')
    rows = c1.fetchall()
    for row in rows:
        if row[1] and row[2]:
            auth_sps['mfa_authz'].add(row[0])
        elif row[1]:
    ...
    arc = config.archive_dir + group['filename'] + '.' + time.strftime('%d')
